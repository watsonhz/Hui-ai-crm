...
def create_acceptance(body: AcceptanceCreate, db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
...
def list_by_project(project_id: int, db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
...
def update_acceptance(a_id: int, body: AcceptanceUpdate, db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
...
def delete_acceptance(a_id: int, db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
    a = db.query(Acceptance).filter(Acceptance.id == a_id).first()
    if not a:
