...
from app.schemas.organization import OrganizationCreate, OrganizationUpdate, OrganizationResponse, OrganizationTreeNode
...
def get_organization_tree(db: Session = Depends(get_db)):
    orgs = db.query(Organization).filter(Organization.deleted_at.is_(None)).order_by(Organization.sort_order).all()
    org_map: dict[int, OrganizationTreeNode] = {}
    roots: list[OrganizationTreeNode] = []
    for o in orgs:
        node = OrganizationTreeNode(id=o.id, name=o.name, org_type=o.org_type, parent_id=o.parent_id, sort_order=o.sort_order)
        org_map[o.id] = node
    for o in orgs:
        node = org_map[o.id]
...
def delete_organization(org_id: int, db: Session = Depends(get_db)):
    ...
    if not org:
        raise HTTPException(status_code=404, detail="组织不存在")
    children = db.query(Organization).filter(Organization.parent_id == org_id, Organization.deleted_at.is_(None)).count()
    if children > 0:
