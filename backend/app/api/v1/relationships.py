...
def create_relationship(body: RelationshipCreate, db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
...
def list_by_customer(customer_id: int, db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
...
def update_relationship(r_id: int, body: RelationshipUpdate, db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
...
def delete_relationship(r_id: int, db: Session = Depends(get_db), user: CurrentUser = Depends(get_current_user)):
    r = db.query(Relationship).filter(Relationship.id == r_id).first()
    if not r:
