...
def update_permission(perm_id: int, body: PermissionUpdate, db: Session = Depends(get_db)):
...
def create_department(body: DepartmentCreate, db: Session = Depends(get_db)):
    if body.parent_id:
        ...
        if not parent:
            raise HTTPException(400, "父级部门不存在")
...
def add_team_member(team_id: int, body: TeamMemberAdd, db: Session = Depends(get_db)):
    team = db.query(Organization).filter(Organization.id == team_id, Organization.deleted_at.is_(None)).first()
    if not team:
        raise HTTPException(404, "团队不存在")
    if db.query(User).filter(User.username == body.username).first():
    ...
    db.add(u); db.commit(); db.refresh(u)
    _log(db, u.id, body.username, "team.member.add", f"团队{team.name}添加成员{body.username}")
